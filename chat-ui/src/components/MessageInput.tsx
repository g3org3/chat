...
export default function MessageInput() {
  ...
  const { mutateAsync, isLoading } = useMutation((payload: any) => api('/api/message', payload), {
    onSuccess() {
  ...
  return <form onSubmit={onSubmit} className="flex gap-1">
    <input ref={inputref} name="message" disabled={isLoading} className="disabled:bg-slate-200 disabled:cursor-not-allowed rounded flex-1 px-3 py-2 text-lg border-2 border-purple-700" placeholder="type your message..." />
